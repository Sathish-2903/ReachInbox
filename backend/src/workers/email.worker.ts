...
export async function recoverStuckProcessingEmails() {
  try {
    const stuckEmails = await prisma.email.findMany({
      where: { status: 'PROCESSING' },
    });
    if (stuckEmails.length > 0) {
      console.log(`[Worker] Found ${stuckEmails.length} emails stuck in PROCESSING. Recovering to SCHEDULED...`);
    ...
    console.warn('[Worker] Failed checking stuck processing emails:', err.message);
  }
}

export async function processEmailJob(job: Job<EmailJobData>, token?: string) {
  const { emailId, recipient, subject, body } = job.data;
  console.log(`[Worker] Job started: job-${job.id} for emailId=${emailId} to=${recipient}`);
  ...
  try {
    ...
    if (!rateCheck.allowed) {
      ...
      // Send Slack notification if connected (non-blocking)
      slackService
      ...
      // Sync state change with Elasticsearch
    ...
    console.log(`[Worker] Email sending: emailId=${emailId} to=${recipient} subject="${subject}"`);
    ...
    await prisma.email.update({
      ...
      data: {
        ...
        // Store rawInfo JSON if preview URL not generated
        // Prisma JSON field not defined, so we store as stringified if needed
        // Using a text column could be added later; for now we ignore if not present
      },
    });

    console.log(`[Worker] Email sent: emailId=${emailId} to=${recipient} | MessageId: ${sendResult.messageId} | Preview: ${sendResult.previewUrl || 'N/A'}`);

    // 8. Update Elasticsearch document to SENT
    elasticsearchService
      .indexEmail({
        id: email.id,
        userId: email.userId,
    ...
    console.error(`[Worker] Email sending failed: emailId=${emailId} error=${error.message}`);
    ...
    try {
      ...
      // Update Elasticsearch document to FAILED
      elasticsearchService
        .indexEmail({
          id: emailId,
          recipient,
      ...
      console.error(`[Worker] Failed updating email ${emailId} to FAILED:`, dbErr.message);
    }
...
emailWorker.on('active', (job: Job<EmailJobData>) => {
  console.log(`[Worker Event] Job started: job-${job.id} for emailId=${job.data.emailId} to=${job.data.recipient}`);
});

emailWorker.on('completed', (job: Job<EmailJobData>) => {
  console.log(`[Worker Event] Job completed: job-${job.id} for emailId=${job.data.emailId}`);
});
...
emailWorker.on('failed', (job: Job<EmailJobData> | undefined, error: Error) => {
  ...
  console.error(`[Worker Event] Job failed: job-${job?.id} for emailId=${job?.data?.emailId}: ${error.message}`);
});
...
// Run startup recovery on worker launch
recoverStuckProcessingEmails();
